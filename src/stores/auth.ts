...
export const useAuthStore = defineStore('auth', () => {
    ...
    const loginWithGoogle = async () => {
        try {
            await GoogleAuth.initialize({
                clientId: '605254666466-ak5ffn87nima68tn2af9va41jq9ljlrq.apps.googleusercontent.com',
    ...
    onAuthStateChanged(auth, (currentUser) => {
        if (currentUser) {
            user.value = {
                ...currentUser,
                photoURL: currentUser.photoURL || 'https://ionicframework.com/docs/img/demos/avatar.svg',
            };
        } else {
            user.value = null;
        }
